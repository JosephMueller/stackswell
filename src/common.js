...
export function rename_text_styles(old_settings, new_settings, document_data) {
    ...
    document_data.layer.layerTextStyles().sharedStyles().forEach(shared_style => {
        shared_styles_by_name.set(shared_style.name(), shared_style);
    });

    old_settings.breakpoint_labels.forEach((old_breakpoint_label, index) => {
        HEADER_TAGS.forEach(header_tag => {
            ALIGNMENTS.forEach(alignment => {
                const old_color = old_settings.naming_convention != "" ? old_settings.naming_convention : `#${old_settings.text_color}`;
                ...
                if (shared_style != null) {
                    ...
                    shared_style.name = `${new_breakpoint_label}/${header_tag}/${new_color}/${alignment}`;
                }
            });
        });
    });
